...
def article_view(request):
    ...
    return render(request, 'pedia/pedia.html', {
...
def add_article(request):
    ...
    if not request.user.is_staff:
        messages.error(
            request, 'You do not have permission to add an article.')
    ...
    if request.method == "POST":
        ...
        if article_form.is_valid():
            ...
            article.save()
            messages.success(
                request, 'Article submitted and awaiting approval')
            return redirect('article_view')
        else:
    ...
    return render(request, 'pedia/add_article.html', {
...
def edit_article(request, article_id):
    ...
    if not request.user.is_authenticated:
        messages.error(
            request, 'You must be at least an admin to edit an article.')
        return redirect('login')

    article = get_object_or_404(
        Article, id=article_id) if article_id else Article()
...
def delete_article(request, article_id):
    ...
    if not request.user.is_authenticated:
        messages.error(
            request, 'You must be at least an admin to delete an article.')
        return redirect('login')
